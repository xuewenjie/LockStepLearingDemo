...
public class FrameScene : GameScene, IReceiverHandler
{
    ...
    private int mFrameInterval = 33;//帧时长
    ...
    public override void OnUpdate()
    {
        ...
        if(Time.frameCount % 30 ==0 && PlayerManager.GetSingleton().mReady)
        {
            ...
            ClientService.GetSingleton().SendTcp(ClientID.Frame, MessageID.GM_PING_CS, sendData);
        }
    }
    ...
    void OnReadyRequest(int data)
    {
        ...
        if (tmpPlayerCharacter)
        {
            ...
            ClientService.GetSingleton().SendTcp(ClientID.Frame, MessageID.GM_READY_CS, sendData);
        }
        else
        {
            Debug.Log("人物没创建完成");
